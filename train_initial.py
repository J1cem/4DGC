...
import sys
from scene import Scene_base, GaussianModel_base
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel_base(dataset.sh_degree,is_train=True, q=dataset.q)
    scene = Scene_base(dataset, gaussians)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene_base, renderFunc, renderArgs):
    if tb_writer:
    ...
    scene.gaussians.is_train = False
